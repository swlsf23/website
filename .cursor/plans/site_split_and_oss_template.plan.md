...
1. Ensure `**content/personal/`** holds all former flat-`content/` (or `personal/`) Markdown; ensure `**content/docs/.gitkeep`** exists. Remove duplicate `personal/` at repo root or root `docs/` if introduced by mistake.
2. Update `**seed_db.py**`, `**render_resume_pdf.mjs**`, `**apps/web/package.json**` resume scripts, `**.gitignore**`, `**content/personal/resume.contact.example.md**` text, `**README.md**` to use `**content/personal/**` and `**content/docs/**`.
3. Later: `git mv` infra → `backend/terraform`, apps → `web` / `backend/api`, etc., per the broader four-part reorg.
